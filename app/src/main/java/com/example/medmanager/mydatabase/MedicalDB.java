...
public class MedicalDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String create_med_table = "CREATE TABLE MEDICINE (" +
                                        ...
                                        "ON DELETE CASCADE);";
        String create_exp_table = "CREATE TABLE EXPIRY (" +
                                        "_id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                                        "MED_NAME TEXT NOT NULL," +
                                        "TIME TEXT NOT NULL," +
...
//    MEDICINE CRUD OPERATION:
    ...
    public void addexpiry(SQLiteDatabase db, @NonNull String med_name,@NonNull String time){
        ContentValues values = new ContentValues();
        values.put("MED_NAME",med_name);
        values.put("TIME",time);
//        values.put("DAYS",days);
        values.put("ENABLE",false);
        db.insert("EXPIRY",null,values);
    }
    public void deleteEXPIRY(SQLiteDatabase db, @NonNull int med_id){
    ...
    public Cursor getexpiryList(SQLiteDatabase db){
        Cursor cursor = db.rawQuery("SELECT _id, MED_NAME FROM EXPIRY",new String[]{});
        return cursor;
    }
...
//    public Cursor getExpiryListById(SQLiteDatabase db,int user_id){
//        Cursor cursor = db.rawQuery("SELECT * FROM MEDICINE WHERE USER_ID="+user_id+";",new String[]{});
//        return cursor;
//    }
    ...
    public Cursor getExpiry(SQLiteDatabase db, int med_id){
        Cursor cursor = db.rawQuery("SELECT * FROM EXPIRY WHERE _id="+ med_id+";",new String[]{});
        return cursor;
    }
}
